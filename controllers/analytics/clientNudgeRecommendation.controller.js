const { getClientNudgeRecommendationModel } = require("../../models/analytics/clientNudgeRecommendation.schema");

/**
 * Get client nudge recommendations with date range filter
 * Path params: clientName
 * Query params: startDate, endDate (ISO 8601 format or YYYY-MM-DD)
 */
exports.getClientNudgeRecommendations = async (req, res) => {
    try {
        ...
        const ClientNudgeRecommendationModel = getClientNudgeRecommendationModel(clientName);
        ...
        const [stats, fullRecords] = await Promise.all([
            ClientNudgeRecommendationModel.aggregate([
                {
                    $match: filter
                },
                {
            ...
            ClientNudgeRecommendationModel.find(filter)
        ...
        res.status(200).json({
            status: true,
            message: "Client nudge recommendations retrieved successfully",
            data: {
                totalRecommendations: result.totalRecommendations[0]?.count || 0,
        ...
        console.error("Error fetching client nudge recommendations:", error);
