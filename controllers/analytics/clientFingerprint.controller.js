const { getClientFingerprintModel } = require("../../models/analytics/clientFingerprint.schema");

/**
 * Get client fingerprint data with date range filter
 * Path params: clientName
 * Query params: startDate, endDate (ISO 8601 format or YYYY-MM-DD)
 */
exports.getClientFingerprints = async (req, res) => {
    try {
        ...
        const ClientFingerprintModel = getClientFingerprintModel(clientName);
        ...
        const [stats, fullRecords] = await Promise.all([
            ClientFingerprintModel.aggregate([
                {
                    $match: filter
                },
                {
            ...
            ClientFingerprintModel.find(filter)
        ...
        res.status(200).json({
            status: true,
            message: "Client fingerprints retrieved successfully",
            data: {
                clientName: clientName,
                totalFingerprints: result.totalFingerprints[0]?.count || 0,
        ...
        console.error("Error fetching client fingerprints:", error);
